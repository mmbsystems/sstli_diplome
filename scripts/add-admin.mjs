...
const [username, password, name, ...extra] = process.argv.slice(2);
if (extra.length || !username || !/^[a-zA-Z0-9_.-]{1,64}$/.test(username) || !password || password.length < 12 || Buffer.byteLength(password) > 72 || !name?.trim() || name.length > 120) {
...
try {
  ...
  const admin = {username, name: name.trim(), passwordHash: await bcrypt.hash(password, 12)};
